...
namespace LeetCodeExample.Test.Common
{
    ...
    {
        private List<T> data;
        private IComparer<T> _customCompare = null;
        ...
        public void Enqueue(T item)
        {
            ...
            while (ci > 0)
            {
                ...
                int cmp = _customCompare.Compare(data[ci], data[pi]);
        ...
        public T Dequeue()
        {
            ...
            while (true)
            {
                ...
                if (rc <= li && (_customCompare.Compare(data[rc], data[ci])) < 0) // if there is a rc (ci + 1), and it is smaller than left child, use the rc instead
                    ci = rc;
                int cmp = _customCompare.Compare(data[pi], data[ci]);
        ...
        public bool IsConsistent()
        {
            ...
            for (int pi = 0; pi < data.Count; ++pi) // each parent index
            {
                ...
                if (lci <= li && (_customCompare.Compare(data[pi], data[lci])) > 0) return false; // if lc exists and it's greater than parent then bad.
                if (rci <= li && (_customCompare.Compare(data[li], data[rci])) > 0) return false; // check the right child too.
            }
